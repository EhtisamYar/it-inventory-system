...
const Employees = ({ apiUrl }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`${apiUrl}/api/employees/${editing.id}`, formData);
      } else {
      ...
      <div style={styles.frame}>
        ...
        <header style={styles.topbar}>
          <div style={styles.brandBlock}>
            ...
            <div>
              <h1 style={styles.brandTitle}>Employees</h1>
              <p style={styles.brandSub}>Manage staff and their details</p>
            </div>
          </div>
        ...
        <div style={styles.statStrip}>
          <div style={styles.statBlock}>
            <span style={styles.statValue}>{String(employees.length).padStart(3, '0')}</span>
            <span style={styles.statLabel}>Total Employees</span>
          </div>
          <div style={styles.statDivider} />
          <div style={styles.statBlock}>
            <span style={styles.statValue}>{String(departments.length).padStart(2, '0')}</span>
                  ...
                  <tr>
                    <td colSpan="9" style={styles.emptyCell}>
                      <div style={styles.emptyWrap}>
                        ...
                        <p style={styles.emptyText}>Click "Add Employee" to create one.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  employees.map((emp, i) => (
                    <tr key={emp.id} className="gl-row">
                      <td style={{ ...styles.td, ...styles.tdMono, color: '#B9B3A4' }}>{String(i + 1).padStart(3, '0')}</td>
                      <td style={{ ...styles.td, fontWeight: 600 }}>{emp.name}</td>
                      <td style={{ ...styles.td, ...styles.tdMono }}>{emp.employee_id}</td>
                      ...
                      <td style={{ ...styles.td, textAlign: 'right' }}>
                        <div style={styles.actionRow}>
                          ...
                          <button className="gl-icon-btn gl-icon-delete" style={styles.iconBtn} onClick={() => handleDelete(emp.id)} title="Delete">
                            <FaTrash size={12} />
                          </button>
                        </div>
                      </td>
        ...
        <div style={styles.overlay} onClick={() => setShowModal(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h2 style={styles.modalTitle}>{editing ? 'Edit Employee' : 'New Employee'}</h2>
            ...
            <form onSubmit={handleSubmit} style={styles.modalBody}>
              <div style={styles.formRow}>
                ...
                <div style={styles.formGroup}>
                  ...
                  <input style={styles.input} type="text" value={formData.employee_id} onChange={e => setFormData({...formData, employee_id: e.target.value})} required />
                </div>
              </div>
              <div style={styles.formRow}>
                <div style={styles.formGroup}>
                ...
                <div style={styles.formGroup}>
                  ...
                  <input style={styles.input} type="text" value={formData.contact_no} onChange={e => setFormData({...formData, contact_no: e.target.value})} />
                </div>
              </div>
              <div style={styles.formRow}>
                <div style={styles.formGroup}>
                  <label style={styles.label}>Designation</label>
                  <input style={styles.input} type="text" value={formData.designation} onChange={e => setFormData({...formData, designation: e.target.value})} />
                </div>
                <div style={styles.formGroup}>
...
const styles = {
  ...
  statDivider: { width: '1px', height: '28px', background: 'rgba(255,255,255,0.14)' },
  ...
  modal: { background: '#fff', borderRadius: '14px', width: '100%', maxWidth: '600px', maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 60px rgba(20,22,31,0.25)' },
